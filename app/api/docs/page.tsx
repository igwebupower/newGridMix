...
type ErrorColor = 'green' | 'yellow' | 'orange' | 'red';

// Tailwind needs static class names, so colors are looked up rather than
// interpolated into template literals (interpolated classes never get
// generated by the compiler unless they happen to appear literally elsewhere).
const ERROR_CARD_CLASSES: Record<ErrorColor, string> = {
  green: 'bg-green-50 dark:bg-green-900/20 border-green-200 dark:border-green-800',
  yellow: 'bg-yellow-50 dark:bg-yellow-900/20 border-yellow-200 dark:border-yellow-800',
  orange: 'bg-orange-50 dark:bg-orange-900/20 border-orange-200 dark:border-orange-800',
  red: 'bg-red-50 dark:bg-red-900/20 border-red-200 dark:border-red-800',
};
const ERROR_BADGE_CLASSES: Record<ErrorColor, string> = {
  green: 'bg-green-500',
  yellow: 'bg-yellow-500',
  orange: 'bg-orange-500',
  red: 'bg-red-500',
};
const ERROR_STATUS_CLASSES: Record<ErrorColor, string> = {
  green: 'text-green-900 dark:text-green-100',
  yellow: 'text-yellow-900 dark:text-yellow-100',
  orange: 'text-orange-900 dark:text-orange-100',
  red: 'text-red-900 dark:text-red-100',
};
const ERROR_DESC_CLASSES: Record<ErrorColor, string> = {
...
export default function APIDocsPage() {
  ...
  const mcpTools = [
    { name: 'get_current_mix', description: 'Live UK generation mix (wind, gas, nuclear, etc.), carbon intensity, and interconnector flows.' },
    { name: 'get_stats', description: 'Current renewable %, fossil %, nuclear %, low-carbon %, and demand.' },
    { name: 'get_intensity_history', description: 'Carbon intensity for each settlement period over the last N hours (1-168).' },
    { name: 'get_intensity_forecast', description: 'Carbon intensity outlook for the next N hours (1-168).' },
    { name: 'get_solar', description: "Solar generation: current snapshot, today's curve, or yesterday's curve." },
    { name: 'get_frequency', description: 'Current grid frequency in Hz and stability status.' },
    { name: 'get_price', description: 'Current GB wholesale system price in GBP/MWh.' },
    { name: 'get_health_score', description: "GridMix's composite Grid Health Score (0-100, with letter grade)." },
    { name: 'get_historical_year', description: 'Annual summary (renewables, price, demand, carbon) for a year, 2000-2025.' },
    { name: 'get_historical_month', description: 'Monthly snapshot for a given year and month, 2000-2025.' },
    { name: 'get_historical_record', description: 'Highest or lowest value of a metric across the full historical archive.' },
  ];
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-900 dark:to-gray-800">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <motion.div
          ...
          className="glass p-4 sm:p-6 lg:p-8 mb-6 sm:mb-8"
        >
          ...
          <p className="mt-6 text-sm text-gray-500 dark:text-gray-400">
            All endpoints return JSON and support CORS. No authentication needed.
            Cache responses for 30 seconds to optimize performance.
          </p>
        </motion.div>
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-4">
                    <div className="flex-1">
                      <div className="flex flex-wrap items-center gap-2 sm:gap-3 mb-2">
                        <span className="px-3 py-1 bg-blue-600 text-white text-xs font-bold rounded-md whitespace-nowrap">
                          {endpoint.method}
                        </span>
                    ...
                    <button
                      ...
                      className="w-full sm:w-auto px-4 py-2 bg-green-600 hover:bg-green-700 text-white rounded-lg font-medium transition-colors flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed whitespace-nowrap"
                    >
                      {loading[endpoint.id] ? (
                        <>
        ...
        >
          ...
          <p className="text-sm sm:text-base text-gray-600 dark:text-gray-400 mb-6">
            Connect Claude, Claude Desktop, or any MCP-compatible client directly to GridMix.
            The server exposes the same tools Watt uses internally as native tool calls, so an
            assistant can query live and historical UK grid data instead of guessing.
          </p>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6 mb-6">
            <div>
              <h3 className="text-lg font-semibold text-gray-800 dark:text-gray-200 mb-3">Endpoint</h3>
              <div className="relative">
                <code className="block bg-gray-900 text-green-400 p-4 rounded-lg font-mono text-sm break-all">
                  https://gridmix.co.uk/api/mcp
                </code>
                <button
                  onClick={() => copyToClipboard('https://gridmix.co.uk/api/mcp', 'mcp-url')}
                  className="absolute top-2 right-2 p-2 bg-gray-800 hover:bg-gray-700 rounded text-gray-400 hover:text-white transition-colors"
                  title="Copy to clipboard"
                >
                  {copiedCode === 'mcp-url' ? (
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-2">
                Streamable HTTP transport, stateless. Standard MCP methods: <code>initialize</code>, <code>tools/list</code>, <code>tools/call</code>.
              </p>
            </div>

            <div>
              <h3 className="text-lg font-semibold text-gray-800 dark:text-gray-200 mb-3">How to Connect</h3>
          ...
          <h3 className="text-lg font-semibold text-gray-800 dark:text-gray-200 mb-3">Available Tools</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-6">
            {mcpTools.map((tool) => (
              <div key={tool.name} className="bg-gray-50 dark:bg-gray-800/50 rounded-lg p-3">
                <code className="text-sm font-mono text-blue-600 dark:text-blue-400 font-semibold">{tool.name}</code>
          ...
          <div className="flex gap-2 mb-4 sm:mb-6 overflow-x-auto pb-2 scrollbar-hide">
            {[
              ...
              <button
                ...
                className={`px-3 sm:px-4 py-2 rounded-lg text-sm sm:text-base font-medium transition-colors flex items-center gap-2 whitespace-nowrap ${
                  activeTab === lang.id
            ...
            {[
              ...
              <div key={error.code} className={`p-4 rounded-lg border ${ERROR_CARD_CLASSES[error.color]}`}>
                <div className="flex items-center gap-3 mb-2">
                  <span className={`px-3 py-1 text-white text-sm font-bold rounded ${ERROR_BADGE_CLASSES[error.color]}`}>
                    {error.code}
                  </span>
                  <span className={`font-bold ${ERROR_STATUS_CLASSES[error.color]}`}>
                    {error.status}
                  </span>
                </div>
                <p className={`text-sm ${ERROR_DESC_CLASSES[error.color]}`}>
                  {error.description}
                </p>
              </div>
            ))}
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
            {[
              ...
              return (
                <div key={i} className="border border-gray-200 dark:border-gray-700 p-6 rounded-xl hover:shadow-lg transition-shadow">
                  <IconComponent className="w-10 h-10 mb-3 text-gray-400 dark:text-gray-500" />
        ...
        >
          ...
          <div className="space-y-4 sm:space-y-6">
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400">
              Need higher throughput? Contact us at{' '}
              <a href="mailto:hello@gridmix.co.uk" className="underline hover:text-gray-700 dark:hover:text-gray-300">
                hello@gridmix.co.uk
              </a>
        ...
        >
          <h2 className="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-gray-100 mb-4 flex items-center justify-center gap-2">
          ...
          <div className="flex flex-col sm:flex-row justify-center gap-3 sm:gap-4 px-4">
            ...
            <a
              href="/support"
              className="px-6 py-3 bg-gradient-to-r from-blue-500 to-purple-600 hover:from-blue-600 hover:to-purple-700 text-white rounded-lg transition-all inline-flex items-center gap-2 font-medium shadow-lg hover:shadow-xl"
            >
